export default function migrate(settings) {
  const menuItems = settings.get("Menu_items");
  if (menuItems) {
    const updatedMenuItems = menuItems
      ...
      .map((item) => {
        ...
        if (parts.length >= 2 && parts[1] !== "none") {
          parts[1] = convertIconName(parts[1].toLowerCase());
        }
        return parts.join(", ");
      })
      .join("|");

    settings.set("Menu_items", updatedMenuItems);
  }

  const submenuItems = settings.get("Submenu_items");
  if (submenuItems) {
    const updatedSubmenuItems = submenuItems
      ...
      .map((item) => {
        ...
        if (parts.length >= 3 && parts[2] !== "none") {
          parts[2] = convertIconName(parts[2].toLowerCase());
        }
        return parts.join(", ");
      })
      .join("|");

    settings.set("Submenu_items", updatedSubmenuItems);
  }

  const svgIcons = settings.get("Svg_icons");
  if (svgIcons) {
    const updatedSvgIcons = svgIcons
      .split("|")
      .map((icon) => convertIconName(icon.trim()))
      .join("|");

    settings.set("Svg_icons", updatedSvgIcons);
  }
